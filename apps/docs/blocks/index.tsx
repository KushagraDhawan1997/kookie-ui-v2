...
export const BLOCKS: readonly BlockEntry[] = [
  {
    ...
    title: "Specimen",
    blurb:
      "A live component and its source code, shown as one figure. If the example has more than one file, each file gets a tab. A ground holds the running component on paper, with its code below it. Thus the pair looks like one thing and its source, not like two unrelated boxes. This arrangement follows the rule of the system for grounds and objects. The surface sizes give the ground larger corners than the paper, so you do not pick a number. The code half is the full code-sample block, so it includes annotations, the copy button, line numbers and the line limit. If the example is already a panel, say so, because a card on paper is the problem that this figure avoids.",
  ...
  },
  {
    ...
    title: "Code sample",
    blurb:
      "A code figure with a label, a copy button and annotations. A quiet header row shows the language or the file name, and the copy button confirms on its own label. The code sits in a recessed area that scrolls sideways and does not wrap. The author can highlight a line or a word, dim all but the part under discussion, and mark the added and removed lines of a diff. These comment notations never go to the clipboard. Long files get a line limit that scrolls and does not clip, with an expand button when the limit applies. Shiki highlights the code with the text colours of the system, so each colour is a value that the generator already checked against the surface below it.",
    files: [
      "code-sample.tsx",
      "copy-button.tsx",
      "expandable.tsx",
  ...
  },
  {
    ...
    title: "Empty state",
    blurb:
      "The content that an area shows when it has nothing to show. It has a mark, a title that names what is missing, a sentence, and one action. The file sets the arrangement and the order of importance. It has two named action slots, not a list, because three buttons in an empty state is the problem that this block prevents. The type enforces this limit. The block has no reason prop. An area can be empty for three reasons: nothing exists yet, a filter matched nothing, or something failed or is not yours to see. These reasons differ in words and in importance, not in arrangement. So they are demos and text, not a switch. Many libraries show “Create your first project” under a search that returned nothing, and the second demo below shows the alternative. The block draws no panel and sets no height. It centres in the area that holds it, and it fits its content when that area has no height.",
  ...
  },
  {
    ...
    title: "Footer",
    blurb:
      "The last area of a page, with a mark, columns of links and a line at the bottom. It uses data, not a set of wrapper parts, because a column of links is a list, so it is a prop. The file holds the arrangement. The block also names each column. Every column is a separate navigation region with its own title as its label. Thus a screen reader announces where each column goes, and does not read five navigation regions with no name. Links are muted at rest and use the full text colour under the pointer. The underline is transparent at rest, so nothing moves when it appears. A footer with no columns is one line: the mark moves down into the bottom row. That is the minimum shape of every app footer, and the data decides it, not a prop. The footer draws no panel of its own, because the page decides what a footer sits on. If you want a footer on a ground, wrap it in a Surface. Icons, a newsletter form and payment marks are not included, for stated reasons. The file is yours, so you can add them to it.",
  ...
  },
  {
    ...
    blurb:
      "The headings of the current page, in the margin, with a mark on the heading that you are at. It works like Tabs turned vertical: a list of places where exactly one is current. Text colour and a line mark the current heading, never a fill. A fill would make it a row that you pick from a list, and the sidebar on the other side of the page is already that. The entries draw the line themselves, with no measured indicator under them. Thus the lit segment is exactly the entry that it marks, and nothing must stay in sync. Sub-headings start further in, and they do not use a third text colour. If you do not control it, it watches the page with an IntersectionObserver and marks the last heading that you passed. A simple spelling that marks the top visible heading shows nothing in that case. If you give it a `current` value, it marks that heading. It does not decide the column. The width, the sticky position and the window width at which two columns no longer fit are part of the page layout. They come from the class that you pass in.",
  ...
  },
  {
    ...
    title: "Conversation",
    blurb:
      "A conversation with an agent, as a screen: the transcript and the parts in it. The transcript opens at its end and follows a reply while you are at the end. When you scroll up, it stops following. It anchors each of your messages near the top, so you read each reply from its start. A button takes you back to the latest message. Under this, it is a MessageScroller on the scroller of the pane. Your message sits on the right on a tint, with its attachments above it. The reply of the agent has no bubble. The tool calls between replies fold into one line. While they run, the line tells you what happens. When they are done, it tells you how much happened. The pictures that a turn made stay out of the fold. When the agent wants to spend something, it stops and asks at the composer, as a Confirmation in its notices. The transcript records only the answer. A failure that ends the turn is a notice with the one action that fixes it. Every word comes from the app, including the counts, because a copied file cannot localise.",
