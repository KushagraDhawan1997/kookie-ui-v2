...
export const ENTRIES: Entry[] = [
  ...
  },
  {
    ...
    axes: [
      { name: "flush", values: "boolean, per pane (default true)", note: "is this pane part of the app frame — tiled with one-hairline seams, and PAINTING NOTHING: flush means level with the page, so the pane has no fill, no corner and no edge of its own, and the app's page shows through. One exception the app never states: a pane presenting as an overlay is not in the frame while it does so, so a drawer takes the surface identity back. Turn it off and the pane floats over the content or grounds beside it, whichever is true; you never state which, and a floating pane picks up the theme's material because it is the one with something behind it" },
