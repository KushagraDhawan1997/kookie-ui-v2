...
export const ENTRIES: Entry[] = [
  ...
  },
  {
    ...
    axes: [
      ...
      { name: "size", values: "1 | 2 | 3 | 4 (on Shell, and per pane)", note: "the index a pane is priced at — its own PADDING first of all, and then whatever it holds: nav rows, rail squares, the header's row. State it once on Shell and every pane takes it; state it on a pane to overrule the app there, which is how a reading column gets more of a safe area than the list of links beside it. It decides the pane's CORNER too, so a pane pulled off the frame wears the corner a Card of that size wears. On the rail and the header it also decides the pane's extent, because a column of squares is as wide as a square plus its air and a header is as tall as its row plus the same — so at one index a header is exactly as tall as the rail is wide. Never the pane's WIDTH: an extent is content speaking and has no ladder, which is why width is a raw number and this is an index" },
    ],
    refusals: [
    ...
    parts: [
        { part: "ShellHeader", blurb: "The full-width top bar — a real <header> landmark; a header that isn't full-width belongs inside ShellContent. Its height is the chrome's, not its content's: one control row at the pane's index, inside the pane's padding, with anything shorter centred in it rather than defining it — and anything taller growing it, because the row is a floor" },
        ...
        { part: "ShellContent", blurb: "The work area — a real <main>, scrolls itself, takes whatever room the panes leave; the one pane every shell should have. It pads like every pane, so a picture or a canvas that wants the whole box says m=\"bleed\" and a ShellScroll inside it reaches the edges on its own" },
