 ...
 * route, the way the tree marks its current chapter.
 */
import Link from "next/link";
import { usePathname } from "next/navigation";
import { ShellRailItem, ShellRailList, ShellTabBar } from "@kookie-ui/react";

import { GridIcon, HomeIcon, IdeaIcon, SearchIcon, TableIcon } from "../icons";
...
const TABS = [
  ...
  { label: "Components", href: "/components", icon: TableIcon, at: (p: string) => p.startsWith("/components") },
] as const;
...
export function DocsTabBar() {
  ...
  return (
    <ShellTabBar aria-label="Sections" flush={false} backdrop>
      ...
      <ShellRailItem label="Search" onClick={() => document.dispatchEvent(new CustomEvent("kd:search"))}>
        <SearchIcon />
      </ShellRailItem>
    </ShellTabBar>
  );
