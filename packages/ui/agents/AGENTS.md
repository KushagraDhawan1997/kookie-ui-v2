...
- **Shell** — A gap prop; A header position axis; A thin sidebar mode; A close-cascade between rail and sidebar; `peek`; `backdrop` on `ShellContent`; A tab bar derived from the sidebar; A tab bar that changes what the sidebar shows; A floating or stacked presentation value
...
- **Toolbar** — Tone, emphasis and material on the ROW; Leading, centre and trailing parts; A size on the group; A gap prop
