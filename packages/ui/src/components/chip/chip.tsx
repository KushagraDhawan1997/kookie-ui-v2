...
/**
 * A chip (§11's inert atoms, §15): a short word or a count that states what the thing beside
 * it IS right now.
 *
 ...
 * edge and relief, because a key is a physical object. A chip is that same box with neither —
 * flat, because a chip is printed ON the thing it marks rather than sitting on top of it — and
 ...
 * rungs and no `variant`: a chip does not come in loud. Ranking is what emphasis does for
 * actions, and two chips of different loudness on one screen say something about importance
 ...
 *
 * **It does not know where it goes.** Apple's notification chip sits ON its container — a tab,
 ...
 *
 * **What it refuses**: a fill ladder (above), a dismissal (a chip you can remove is a control
...
export function Chip({
  ...
  const merged = {
    ...
    className: className
      ? `kui-type kui-atom kui-atom-box kui-chip ${className}`
      : "kui-type kui-atom kui-atom-box kui-chip",
    style,
    ...props,
  };

  /**
   * AN EMPTY CHIP RENDERS NOTHING (2026-08-23, ultracode audit).
   *
   * The refusal was written and never enforced: `<Chip tone="success">{done && "New"}</Chip>`
   ...
   * `{undefined}` all reached it; only a bare `<Chip/>` was a type error.
   *
   * The TYPE cannot close this, which is why it is a runtime answer: the commonest way to write
   * a conditional chip is `{cond && "New"}`, whose type is `string | false` and which is a
   ...
   *
   * Rendering nothing rather than throwing: a chip that has nothing to say is a chip that
   ...
   * a chip reading "0" is a real thing to say.
   */
  if (!filled(children)) return null;

  // One glass per stack, structurally: a glass chip scopes its subtree solid, and a chip
