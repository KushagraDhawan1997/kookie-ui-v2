...
describe("weight takes token names, never numbers (§15)", () => {
  it("Text rests regular, Heading rests medium, and the prop moves both", () => {
    // Heading rested semibold until 2026-09-19: size ranks, weight does not (§15 rule 1).
    expect(computed(render(<Text>t</Text>), "font-weight")).toBe("400");
    expect(computed(render(<Heading>h</Heading>), "font-weight")).toBe("500");
    expect(computed(render(<Text weight="semibold">t</Text>), "font-weight")).toBe("600");
    expect(computed(render(<Heading weight="semibold">h</Heading>), "font-weight")).toBe("600");
  });

  it("700 is unreachable — semibold is the TOP of the ladder (§15)", () => {
    ...
    for (const el of [
      ...
      <Heading size="9" weight="semibold">h</Heading>,
      <Heading size="1" weight="semibold">h</Heading>,
    ]) {
