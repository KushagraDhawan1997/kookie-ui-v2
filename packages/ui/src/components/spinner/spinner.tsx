...
/** Twelve spokes on a 24-unit grid: a 2-wide rounded bar from radius 4.5 out to radius 10. */
const SPOKES = Array.from({ length: 12 }, (_, i) => ({
  ...
  opacity: (1 - (i / 12) * 0.85).toFixed(2),
}));

/**
 * A busy indicator (§8): twelve spokes with a fading trail, ticking one spoke at a time.
 *
 ...
 * primitive, not a tuning problem. The twelve rects are static: they rasterise once and then
