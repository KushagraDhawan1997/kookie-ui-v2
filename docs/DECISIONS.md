...
**Since 2026-08-10 this is the DEFAULT path, not a hypothetical one** (Kushagra): the shipped `accent` is neutral's own recipe, `{ hue: 250, vividness: 0.04 }` — a grey brand. Every primary button, checked mark and focus ring in the system therefore resolves through the low-chroma branch, and the wider `lowChromaStateScale` is what keeps its three states apart. Going back to a chromatic brand is one config line; the identity that used to make `accent ≡ blue` now makes `accent ≡ neutral`, by pointing at the same recipe rather than by repeating a number.
...
**What is still deferred (2026-08-10):** the floating family's exit (Menu's is open — see LOG), Select's own entry, the slider's grip under a drag, the button's shadow crossfade (blocked on the flat/elevated ruling: a cast that does not exist has nothing to step), and every v0 number here. Select is the nearest: it wears `kui-floating`, so the machinery is one line away, but a select opening is a different gesture from a menu opening and has not been judged. The grammar is settled and the tokens exist; what remains is eye-pass work, and each piece lands with its own laws.
...
**What 1.5 means on the glass, because the number is not pixels.** It is user units inside a 24 viewBox, and the box is `--icon-size-N`, so the painted weight is `1.5 x box / 24`: **1.0px at sizes 1–2** (16px box), 1.25px at size 3, 1.5px at size 4. A constant painted weight is not expressible here — it would need the stroke to change with the box, which CSS cannot compute per instance — so the ladder's thinner strokes at the small end are the shipped behaviour, not an oversight. Optical centering and the set contract stay open with the icon-system decision; the box and the stroke are pinned.
