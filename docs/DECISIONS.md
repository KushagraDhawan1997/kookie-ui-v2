...
**`size` is optional here, with no default, and that is the atom's own rule.** Text anchors at 3; a `Code` that copied that default would push a size-1 caption's inline literal to 16px, and the component would simply stop being used inside small text. Unset means the element states no step and inherits the line it sits in — font size, line height and letter spacing together — so `<Text size="1">…<Code/></Text>` matches by construction rather than by the call site repeating the index. A stated step resolves identically to `Text` at that step (mounted law).
