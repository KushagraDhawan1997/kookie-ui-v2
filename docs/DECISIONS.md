...
| Code / Kbd | neutral *(stamped)* | ladder-as-roles | - | - | subtle fill — SHIPPED 2026-08-08 as type-family atoms: the mono slot + a FIXED fill, `size` optional (unset = the line it sits in); the row's old `medium` named a fill rung and is corrected below |
...
**The mono optical correction (2026-08-08, Kushagra): the atoms render their FONT-SIZE at `monoScale` (v0: 0.925) of the step, and nothing else moves.** Mono faces run wider with a taller x-height, so at equal font-size the chip read a step large beside its own line — the peers all discount (GitHub prose 85%, Radix Themes 0.9em). The split is deliberate: line-height stays the step's line box, so a chip sits in unmoved vertical rhythm; letter-spacing stays the step's `em` value, which follows the discounted glyphs by construction (tracking is a property of the rendered letters — the em-padding argument one property over). Mechanism: the type join multiplies every size arm's font-size by `--kui-ty-scale`, registered non-inheriting with initial 1 — the identity for Text, Heading and Blockquote, byte-equal to the bare token — and the mono atoms stand it down to `var(--mono-scale)`, with an `em` spelling covering the inherited-size arm. Non-inheriting is load-bearing: a Text nested inside a Code takes its full step, not the chip's discount (law-mounted). Rejected: `font-size-adjust`, the principled x-height normalizer — the right aspect constant depends on which platform font the system stack resolved, which config cannot know.
...
**Kbd is Code plus an edge**, and its stylesheet repeats Code's facts self-keyed on purpose: the second member of a family self-keys, the third promotes it into a layer (the field family's rule, LOG 2026-08-05). Badge and Tag are the atoms still to land. The edge is the tone-aware `--tone-border`, not one of the solved tiers — those were solved for controls whose identity *rests* on the hairline (§7's edge order), and a cap has a fill to carry it. No shadow: a key cap is the classic "just a tiny inset shadow" case, and it is refused where depth is the app's (§5).
