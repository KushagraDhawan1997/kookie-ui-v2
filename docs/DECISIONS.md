...
- **Container containment: a shrink-to-fit Box collapses to zero width.** Every Box and Theme is `container-type: inline-size` so tiers can measure it (section 2), and inline-size containment sizes the element *as if it had no contents* — so a Box sitting where the parent would shrink-to-fit it (a row-flex item, `inline-block`, a `max-content` grid track) computes width 0. The generator's own comment has always named this for Theme; the docs app (the first real consumer, 2026-08-05) hit it on Box with the most ordinary composition there is: a header row holding a nested button group. The taught escapes are explicit sizing — `flexGrow` takes width from space distribution, the one route containment leaves open; `width` states it — or restructuring so Boxes are column items, which stretch. Open: whether this stays a documented cost (the Theme precedent), grows a section-2 cost-list entry with a law, or Box someday makes containment opt-out per element — which would trade it against tiers silently not working there, the exact inversion §2 chose containment to avoid.
