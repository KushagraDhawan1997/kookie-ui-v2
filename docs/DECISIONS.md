...
surfaces       flat | elevated                 do surfaces sit up (section 10). SHIPPED 2026-08-04. The semantic is elevation-as-identity; shadow row 2 is its current resolution. An app choice made once - no component exposes a shadow API, law-tested. Fields left its set 2026-08-06 (a field is a well; the criterion sentence lives in section 5's chrome notes)
...
**The mark edge (2026-08-06, Kushagra — audit D2).** A mark is the only control whose resting identity is its hairline alone — a field has a seal and a value, a card has a body, but an unchecked checkbox IS its border — and the shared `--color-border` (neutral 7) measures |Lc| 22.8 light / 10.3 dark against the surface, far under the non-text floor of 45 the system already enforces for the focus ring and the invalid edge. So the family gets its own role: **`--mark-edge`**, resolving per mode to the first neutral step that clears the floor against both the surface and the page — light 9 (Lc 58.8), dark 11 (66.5; dark's own step 9 misses at 42, its ladder folding back past the solid). Per-mode steps are the focus-ring precedent; the picks live in color-config and the law beside the invalid edge's, watched failing at step 7 before the picks were accepted. The scope IS the decision: checkbox now, radio/switch/slider when they land, and **nothing else changes** — re-stepping `--color-border` for everyone was the other branch, refused because a quiet hairline is part of how surfaces read and their identity does not rest on it. `contrast="high"` reaches the mark automatically, since the role resolves through the re-declared neutral scale.
...
- **Theme `surfaces="elevated"`** — the one stylesheet consumer (§5). Each world scope declares `--kui-surface-chrome`, and the elements that read it paint it: `.kui-surface`, and nothing else. **Fields left the set 2026-08-06** (they had joined 2026-08-04 on the sentence "a text field is a bordered box sitting on the page — it lifts with the cards"; the sentence was asserted, not judged): elevation separates a plane from what is BEHIND it, and a field is a **well** — content of a plane, not a plane above one. A recessed thing cannot cast a drop shadow; Material's fields are filled or outlined, never raised, and no platform shades an input. The membership criterion is now one sentence: **elevation dresses boxes that establish a plane of their own**. A control's box is the action (Button) or the state (a mark) or a well (a field) — none of them qualifies, and each family's flatness under `surfaces="elevated"` is a mounted negative law, not an omission. `flat` still resolves to `none`, so the default stays byte-identical to a world without the rule. **The count that matters is not how many rules exist but that every one resolves `--kui-surface-chrome`** — a law walks every shipped stylesheet and asserts exactly that, and with the set at one member it also pins the count again: a second consumer appearing is a decision, and it fails there first. It escapes by declaration, like every other axis: written as a descendant selector with no reset (until 2026-08-03) a nested `surfaces="flat"` matched nothing and the elevated ancestor's rule still reached the inner cards, so a flat panel inside an elevated shell was unbuildable. An app identity, never a per-card knob.

  **One lighting model, the palette its source of truth, and the edge is not special (finesse passes, 2026-08-04, Kushagra).** Three constraints closed it in sequence: the edge should read lighter at top and darker at bottom; whatever consumes shadow must share one lighting model, including shadow itself; and contrast must reach the edge. Resolution — *add depth, change nothing else*: `--surface-chrome` is `var(--shadow-2)` (dark prepends the one thing a drop shadow cannot express, the inset top rim-light a dark fill needs), so tuning row 2 tunes every elevated surface; the border stays the same `--tone-border` every surface wears, which keeps the edge sharp and puts it inside the contrast system for free; the top edge reads lighter because every palette shadow falls downward. Two dead ends recorded so they stay dead: an inset ring beside a transparent border (paints inside the border strip — two lines), and a raw-alpha border (soft, and blind to `contrast="high"`). Laws: exactly one `box-shadow` in the surface layer, inside the elevated scope, reading `--surface-chrome`; the chrome composes `var(--shadow-2)`; mounted, an elevated card's computed shadow equals row 2 exactly and its border is identical to the flat world's.
...
**The slider track is IN the axis — reversed 2026-08-06, the same day.** It was excluded on the sentence "the look trades a border for a fill, and the track never had a border: it is already an edgeless well". That was true of the trade, and the trade is precisely what stopped being the design — once `filled` became a fill question, an untouched rail was just the one part of the slider that ignored the app's identity, which is how Kushagra put it from the preview: *"slider doesn't respect outline at all rn."* It also had a measured cost the exclusion hid: the thumb reads `--look-mark-fill` and the track read `--color-track`, and **both resolved to `--neutral-4` in both appearances**, so a filled slider's handle was its own rail at 1.000:1 — a control that cannot be seen or aimed at. The well now moves *away* from the mark family rather than the mark family moving off a value that suits it: under `filled` the rail sinks (darker than the card in both modes, which is opposite arithmetic per mode), and the handle stays where it reads. `outlined` emits `initial`, so the untouched well role resolves at the element and the default path is byte-identical to a world without the axis.

The general test survives the reversal, restated: the axis has something to say about any part that has a **fill** — and the only parts it has nothing to say about are those with neither a fill nor a boundary of their own. What replaced the old "pinned unmoved" law is a pair: the track must *differ* between looks, and the thumb must never equal its rail in either look.
