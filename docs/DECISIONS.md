...
The ramp and the weights are the *materials*; this is how they get **used**. It is not a token and no law can enforce it — it is the standing brief for every surface this project composes: the playground's real screens, apps/docs, and kookie-blocks when it lands. Recorded because "taste is the last layer" is not the same as "taste is undefined", and because the first two passes at the playground failed on exactly these points.

**The reference set, named so a later reader can look at the same thing:** Rauno Freiberg's site (rauno.me), Pangram Pangram's foundry pages, Awwwards' Sites of the Day and Academy boards. Different subjects, one grammar.

1. **Size ranks, weight does not.** Three weights exist and semibold is the ceiling (above), so the only lever with real range is the ramp. A heading and the paragraph under it should be *far* apart — the references run 3–4× on a page hero and ~1.5–1.8× on a card. A 16px title over a 14px body is 1.14×, which is not a hierarchy, it is a rounding error. When something needs to rank higher, it gets a step, never a heavier face.

2. **The eyebrow is the structural device.** A tiny quiet label sits above a large heading and names the *kind* of thing that follows — "Winners" over "Recent Sites of the Day.", "Academy" over "Learn from the best instructors.", a pill reading "Update" over "Neue Montreal". It carries real information (which is the test §-structure sets for any such device: it encodes something true, or it is decoration), and it is what lets the heading below it be a statement instead of a label. Size 1, quiet, no uppercase and no letterspacing trick — the ramp's own bottom step.

3. **A heading is a sentence, and it ends in a period.** "Recent Sites of the Day." not "Recent sites". Two lines is the good length; `text-wrap: balance` where the break matters. The eyebrow already said what section this is, so the heading is free to say something.

4. **Space is asymmetric around a heading.** Generous above (it separates from what came before), tight below (it binds to what it introduces). Roughly 3–4× above versus below. The common failure is one uniform gap, which makes a heading float between two blocks belonging to neither.

5. **Meta is a labelled row on a hairline, not a sentence.** Awwwards' course cards put `Instructor — Daniele Buffa` and `Score — 4.9/5` on their own divided rows: label left in medium, value right in regular, a rule between. Anything that is a fact about the thing goes there rather than into the prose.

6. **Tracking is already in the ramp — use the top of it.** `--letter-spacing-N` runs 0 to −0.025em (above); the negative tracking only appears at steps 4 and up. A display line that stays at step 5 never gets the tightness the references are built on. This is another reason to reach for a bigger step rather than a heavier one.

7. **One accent, once per screen, at full strength.** Rauno's yellow circle, Pangram's red *Buy Now*, Awwwards' yellow score rule — everything else is ink and grey. Two loud things on one screen is two focal points, which §11 already says about `emphasis`; this is the same rule at composition scale.
