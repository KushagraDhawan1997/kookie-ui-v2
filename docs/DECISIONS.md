...
- **Theme `surfaces="elevated"`** — the one stylesheet consumer (§5). One rule dresses every `.kui-surface` with `--surface-chrome` on the element that owns the radius; `flat` (default) is byte-identical to a world where the rule does not exist. An app identity, never a per-card knob.
...
### Material on a control (2026-08-04): glass owns the fill, tone rides the label

Deferred at §14 step 5, closed when material returned to Button. **While a material is on, it owns the control's whole fill machinery — rest, hover and active — and the rung's dressing sleeps until it comes off.** Glass over an arbitrary backdrop is a different legibility problem from a fill over the page, and the rung fills were designed for the page; letting a rung tint the glass would be a tone × material product the token layer never pays. What survives of the axes:

- **Tone rides on the label.** The glass fill is neutral (mode-aware, mixed over `--neutral-1`), and the label re-scopes to `--tone-label` — `--tone-contrast` is APCA-paired to `--tone-solid`, a fill that is no longer there. An accent glass button is a neutral glass with an accent label, which is also the platform's own answer (iOS tints the symbol, not the glass).
- **Interaction steps glass's one ramp: the mix percentage.** §8's +1/+2 rule translated — each thickness carries designed `[rest, hover, active]` alphas, stepping toward the seal, monotone per column so thickness reads as one dimension mid-interaction. The filter never moves with state: a blur change re-samples the backdrop and shimmers.
- **The opaque environments respond through the seal's steps.** Where `backdrop-filter` is unavailable or transparency is reduced, the fill is `--material-opaque-fill` and hover/press read `--color-surface-hover/-active` — the near-opaque fallback is the seal's neighbour, so it responds like the seal does.

The Theme policy clamp (still open, below) is what makes this reversible tree-wide: forcing solid returns every control to its rung.
