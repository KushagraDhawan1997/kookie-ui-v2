...
**Kbd shares Code's fill and tone facts in its OWN family and box (revised 2026-08-08, the day after it shipped — Kushagra, judged in the playground).** It shipped as "Code plus an edge" wearing the mono slot, and the mono slot is what made ⌘ read too small: a mono cell draws symbols compact to fit its fixed advance. The platform answer decided it — macOS menus set shortcuts in the UI sans, and Radix's cap is sans at 0.75–0.8em — so the cap wears the **body family** at its own deeper factor (`kbdScale`, v0: 0.85, below the chip's 0.925 and law-asserted apart from it), with a cap geometry: `inline-block`, never wraps, `min-inline-size: 1.6em` so a one-glyph key stands near-square and centered, and its own snug line-height so an inherited cap never spreads the line it sits in (law-measured at the small steps, where the line box has the least room). What survives of the family identity: the fill, the tone indirection, the type-join membership, and the edge — tone-aware `--tone-border`, not one of the solved tiers (those were solved for controls whose identity *rests* on the hairline, §7's edge order; a cap has a fill to carry it). No shadow: the classic "just a tiny inset shadow" case, refused where depth is the app's (§5). Stylesheets stay self-keyed — the second member self-keys, the third promotes (LOG 2026-08-05); Badge and Tag are the atoms still to land.
