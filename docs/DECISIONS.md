...
**Each family says how far it moves; nobody says how long.** A button SINKS (down and smaller: only-down reads as sliding, only-smaller as receding), a mark SQUASHES (it has no depth to sink into), a field does nothing. The clock is the skeleton's, the distance is the family's — which is what keeps a press one sentence per component.
...
**The ring LANDS, and only where that answers something.** After a Tab the eye does not know where focus went, so the ring contracts into place — catchable in peripheral vision, where one that simply appears is not. After a click it knows exactly, so the same motion is decoration. No JS is needed because `:focus-visible` already draws that line — except on text-entry elements, where browsers match it on click too, so those are named out and a field's ring is instant.
...
**Exactly one stylesheet moves, and a law names it.** The old zero-transition law walked every sheet, which is what kept motion from being accreted one component at a time; motion did not remove that need, it narrowed it. `menu.css` is the only sheet with a `transition`, because the emergence recipe is the only motion that has been judged. A new component easing something on its own fails in CI, not in an audit six weeks later.

**Suppression is total.** Under `prefers-reduced-motion: reduce` the panel is simply there and then simply gone — no travel, no reveal, no dissolve — and the JS measurement that only serves the animation is not taken either. A suppression that leaves one channel moving is worse than none: the user who asked for stillness gets it in pieces.
...
**What is still deferred (2026-08-09):** the floating family's exit (Menu's is open — see LOG), Select's own entry (it wears `kui-floating`, so the machinery is one line away, but a select opening is a different gesture and has not been judged), the slider's grip under a drag, and every v0 number here. The grammar is settled and the tokens exist; what has not been judged is what a switch's thumb, a button's press or a field's focus should do with it. Those are transcriptions of recipes already judged in the motion lab, not new searches — but they are eye-pass work, and each lands with its own laws. Select is the nearest: it wears `kui-floating` and gets no motion at all today, deliberately, because a select opening is a different gesture from a menu opening and has not been judged.
