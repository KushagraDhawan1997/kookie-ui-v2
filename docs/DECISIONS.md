...
**Interaction stays instant.** Every state change on a control — hover, press, release, disable — is instant, on both pointer worlds, and the shared control layer contains no `transition` at all. That is not a placeholder any more; it is the finding below, kept.
