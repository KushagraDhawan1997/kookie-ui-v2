...
- **The deal, named:** nothing in-app can sit above a portal by z-index. That is the point — an open popup is the thing being interacted with — and the escape is symmetric: what genuinely outranks popups (a banner, a call window) portals too, making "above everything" an explicit act instead of a bigger number.
