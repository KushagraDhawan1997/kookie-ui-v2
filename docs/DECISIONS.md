...
- **The cap is what answers the overlap objection rather than arguing with it.** The raw ladder reaches 68px at comfortable coarse, which would be 20px of invisible reach on every side of a 28px mark. Past 44 a target buys nothing and starts taking from its neighbour. With the cap the widest reach in any of the 24 cells is **10px**, law-tested, so a stacked list is clear at any gap the layout-space scale offers.
...
**A mark's corner has a ceiling, because a circular checkbox is a radio (2026-08-05).** The control band states the capsule at `full`, and shape is role semantics for the mark family — role legibility outranks theme uniformity. `--radius-mark-cap-N` is what the corner would be at the system's **default** level, so a theme can round a mark right up to "what everything looks like when nobody asks for round" and no further; the consuming layer takes `min(theme corner, ceiling)`. `full` and `large` both land on it, `small` and `none` pass through untouched — which is what keeps the kill switch above whole. Derived rather than designed, one length that no axis moves, so there are no per-level cells. Rendered 4/6/8/10 against marks of 16/20/24/26 — between 0.25 and 0.38 of the box, law-tested to stay under half (half IS a circle). v0 for the eye pass.
...
- **Theme `surfaces="elevated"`** — the one stylesheet consumer (§5). Each world scope declares `--kui-surface-chrome`, and the elements that read it paint it: `.kui-surface`, and since 2026-08-04 `.kui-field` (a text field is a bordered box sitting on the page — it lifts with the cards, or the identity is only half applied). `flat` still resolves to `none`, so the default stays byte-identical to a world without the rule. **The count that matters is not how many rules exist but that every one resolves `--kui-surface-chrome`** — a law now walks every shipped stylesheet and asserts exactly that, because the earlier "exactly one `box-shadow`" claim counted a single file and went stale the moment a second element legitimately needed depth. Consequence worth knowing: in an elevated world a field inside a card wears the same row 2 the card does, which is the nesting behaviour two stacked cards already had. It escapes by declaration, like every other axis: written as a descendant selector with no reset (until 2026-08-03) a nested `surfaces="flat"` matched nothing and the elevated ancestor's rule still reached the inner cards, so a flat panel inside an elevated shell was unbuildable. An app identity, never a per-card knob.
...
- **Radio is the checkbox's shape sibling, and it is what should promote the family.** Same mark, circular corner (the one control in the band that IS a capsule), same target rule, `name`-grouped. The mark box rules deliberately sit in `checkbox.css` until a THIRD member exists — TextArea's rule for the field family, applied here (LOG 2026-08-05).
- **Switch leaves the control height ladder — and now has somewhere to land (amended 2026-08-05).** It takes the mark family at a one-index shift (§4) rather than a bespoke designed pair, so a switch and the checkbox above it in one form cannot drift apart; the THUMB is that track minus a designed inset, and the WIDTH is still genuinely open (Material 1.63x, iOS 1.65x, Radix 1.75x — all fractions, so ours will be a designed number). Known wrinkle to settle when it ships: the handheld band collapses type steps 4 and 5 onto one value, so the shift is inert at size 4 under coarse, where a switch would stand exactly as tall as the checkbox. The original pre-scoping — its own designed pair (track × thumb) per pointer world — it does not take control height on iOS either, and coarse widens the track/thumb (iOS 26 direction) to buy target area the height ladder would not give it. The numbers are config lines, judged in the preview; a taste call lands here with the coarse pair especially.
