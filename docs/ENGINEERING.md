...
- **A portalling component wraps its portal content in `<Theme>` and owes the agreement law** (§20, 2026-08-09). The tokens are attributes on the Theme element and a portal lands outside all of them; the bare wrapper re-resolves through React context and re-stamps. The owed law is the two-implementations rule applied to portals: the portalled part must compute identical to an in-flow twin under a hostile axis set, `contrast="high"` included. The frame (`isolation: isolate` on the DOM-outermost theme) is the system's half and already law-pinned (`theme/portal.browser.test.tsx`, the node spelling law, the dist-survival assertion in `build-css.mjs`); the wrapper is the component's half, per component. *Enforced by: `components/menu/menu.browser.test.tsx`'s agreement law (per component, per this section's own "sufficiency cannot be walked" — Menu landed the pattern 2026-08-09; each new portalling component writes its own).*
