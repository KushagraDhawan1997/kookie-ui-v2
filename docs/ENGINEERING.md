...
- **Dress yields to state:** a component's own variant rules use `:where()`, so `invalid`/`disabled` (which use plain specificity) outrank them by source order instead of tying. *Law-shaped only per component (checkbox has the mounted pattern); no structural law — marked.*
...
- **What fires without you:** the directory-walked laws — no axis in component CSS, no raw px, no self-invented shadow, ring tokens on every focus rule, no layout-owned stem, and this contract's own walks — audit a new stylesheet the day it exists. **What does not:** the mounted behavioural laws (the 2026-08-03 standard: computed values through a `<Theme>`, both appearances) are still written per component, and a component without a browser test file has none — marked, no law asserts the file exists.
