...
3. **Types are the refusals, enforced, and a refusal states its own escape.** What the system forbids is unexpressible, not warned about — closed unions everywhere a scale exists. Since §48 a refused prop IS in the props type, branded so no value satisfies it and carrying the sentence that names what to write instead: `<Button m="4">` prints "A component sets no outer spacing. Wrap it — `<Box m="4"><Button/></Box>`" rather than an anonymous "not assignable". One name is the exception and the reason is structural: React's own `HTMLAttributes` declares `color?: string`, so a component extending a native element's props has to keep that key assignable inbound or every wrapper forwarding `ComponentPropsWithoutRef<"button">` stops compiling. `color` is omitted from the type and refused by the ESLint plugin's `no-refused-prop`, out of the same table.
...
- **`apps/docs` reaches `packages/ui` through the package entry, never by relative path into `src/`.** A type-only import is erased before Node runs, but TypeScript still LOADS the file it names, so one relative import pulls package source into the app's program — where `allowImportingTsExtensions` is false, and every extension-bearing import inside the package fails the app's build with `TS5097` in files the app never mentions (2026-09-07). Reading a package file's TEXT is fine and is what several laws do; it is the module graph that must stop at the entry. A value the app needs and the entry does not export is an export decision, not a reason to reach past it.
...
- **The axes are stamped as data-attributes** (`data-size`, `data-tone`, `data-emphasis`, `data-bordered`, `data-material`, plus states) — and `solid` material writes **no** attribute: the layer's material selectors match the three thicknesses plus `on-glass` — a RESOLVED value only nesting can produce (2026-08-16, §10), never authorable, carried in `recipes.test.ts`'s named `RESOLVED_ONLY` exemption. An identity a component fixes (a field's neutral tone) is still stamped; a prop the component refuses is not the same as an attribute it omits. *Enforced by: the mounted component laws; the attribute vocabulary lives in ENGINEERING §3 and `system/axes.ts`.*
- **`--kui-ct-*` stems come in three kinds, and only one is yours.** Layer-private (declared and read by `recipes.css` — do not touch), layer-to-component (`--kui-ct-mark`, `--kui-ct-slot-inset`, `--kui-ct-hosted-height` — read if the geometry applies), and component-owed: a control that re-sources its fill declares **all three** of `--kui-ct-fill-src` / `-src-hover` / `-src-active` and pairs them with `--kui-ct-label-color`. **Corrected 2026-08-26 — the three SOURCES are not registered and therefore INHERIT** (only the derived trio `--kui-ct-fill` / `-hover` / `-active` carries `@property { inherits: false }`, recipes.css). So a partial re-source does not paint transparent; the missing member silently takes the nearest ancestor control's value — the `--kui-h` trap, parent control to child control, which is the shape that put an opaque fill on a glass segmented track (2026-08-24). The fill-triple law is what makes the partial re-source unwritable; the registration is not. *Enforced by: the fill-triple law (per rule, every sheet).*
...
- **Wiring:** the stylesheet is `@import`ed by `styles/index.css` (*enforced: entry-walk law, both directions — the suite's `?raw` list is pinned against the entry for MEMBERSHIP, and since 2026-08-26 for ORDER too: `test/cascade.test.ts` compares the two lists position by position, after six of thirty-four positions had drifted under a `toContain` that never reads an index*), and any new Base UI entry point joins `vitest.config.ts` `optimizeDeps.include` (*enforced: pre-bundle law; the prose rule in §7 stands as the reason*).
...
- **A portalling component wraps its portal content in `<Theme>` and owes the agreement law** (§20, 2026-08-09). The tokens are attributes on the Theme element and a portal lands outside all of them; the bare wrapper re-resolves through React context and re-stamps. The owed law is the two-implementations rule applied to portals: the portalled part must compute identical to an in-flow twin under a hostile axis set, `contrast="high"` included. The frame (`isolation: isolate` on the DOM-outermost theme) is the system's half and already law-pinned (`theme/portal.browser.test.tsx`, the node spelling law, the dist-survival assertion in `build-css.mjs`); the wrapper is the component's half, per component. *Enforced by: `components/menu/menu.browser.test.tsx`'s and `components/select/select.browser.test.tsx`'s agreement laws (per component, per this section's own "sufficiency cannot be walked" — Menu landed the pattern 2026-08-09, Select inherited it the same day through `system/floating.tsx`, the machinery's promoted home; each new portalling component writes its own).*
- **An interactive row wears `kui-control kui-row` and stamps its fixed identity** (§21, 2026-08-09): `data-emphasis="quiet"` always (emphasis is refused on rows), `data-size` from its component's context, `data-tone` neutral unless the row carries the one meaning (`destructive`). The family's box (the text line plus the designed row inset — never the height ladder), lit state and checked indicator accent live in `recipes.css` and nowhere else. *Enforced by: the row-family walk (recipes.test.ts — no component sheet restates the box or the lit state; the shared block exists once).*
...
The rules this section carried for laws that watched animations — stand-downs that must win, instants derived from the runner, instruments armed before the flight, and the frame-watching laws kept off CI — left with motion on 2026-09-20; `docs/archive/motion-v1.md` records them.

**A driver gesture resolving is not the browser having settled — and the rule is ENFORCED, not remembered (2026-08-21).** `await userEvent.click(...)` followed on the next line by an `expect` reading what the click causes asserts that the effect is SYNCHRONOUS: `data-popup-open` is React state committed in a render, a popup unmounts after Base UI finishes closing it, `:hover` is the browser's answer for an element something may still be covering. True on an idle laptop, false on a starved runner — five CI failures in two days. The previous round swept all eight instances that existed and it came back anyway, because the next week's work wrote three more; the sharpest evidence is that `dialog.browser.test.tsx`'s `warnings()` helper had already been rewritten to WAIT while its twin fifty lines away in `alert-dialog.browser.test.tsx` still slept 60ms, and that sleep is the one CI caught. **A lesson that lives only in the lines it repaired stops at the file it was learned in.** `src/test/settling.test.ts` fails the shape at authoring time in milliseconds on any machine; the repair is always "wait for the state, then assert it", and the one exception — a claim about a NON-event, where waiting could only delay a correct answer — is marked `// SETTLED-BY-DESIGN: <why>` with a reason the law holds to a real sentence.
