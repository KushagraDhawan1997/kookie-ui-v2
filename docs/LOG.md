...
Ultracode, 17 agents (eight lenses, eight skeptics, one completeness critic), every finding re-measured in a mounted browser. 44 raised, 33 survived, seven repairs. DECISIONS §26 carries the shipped summary; this records what the day actually taught.
...
**Left open rather than patched, with the reasoning recorded** (both in §26): containment stops at the shell root, so a shell that is not the whole page leaves the surrounding page tabbable while a pane overlays — that is a design question (does a shell overlay trap like a dialog, or IS a shell the page?) and the pointer half is already covered by the scrim; and containment is post-mount, so a server-rendered `defaultOpen` overlay is uncontained for keyboard and AT until hydration, the same honest-null shape §18 already takes. Neither is patched quietly, because a half-answer to either would be indistinguishable from the answer.

## 2026-08-16 The shell ships, and the building closed three questions the spec left open

Same day as the spec (§26 carries the shipped summary; this entry records what was decided rather than executed). The build was the spec followed to the letter plus three genuinely open calls:
...
DECISIONS §26. The session read v1's Shell whole — 4,300 lines of TS, 643 of CSS, 46 test files, plus its own audit report with open P0s — and the spec that came out is mostly subtractions, each one removing a fact that lived in two places.
