...
**Still open and honest** (§26): containment is post-mount, so a server-rendered `defaultOpen` overlay is uncontained for keyboard and AT until hydration — the same honest-null shape §18 already takes, and not patched quietly, because a half-answer would be indistinguishable from the answer.
