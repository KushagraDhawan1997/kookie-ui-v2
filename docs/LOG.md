...
**Law 3 — "both channels actually MOVE" — was scanning a discontinuity it created.** The loop reads the rect and THEN asks whether the flight is on, so its final sample is taken with the pins already stripped and the box already in its natural layout. That step is instantaneous, which is the one shape pixels-per-millisecond cannot normalise — CI read `1.58px/ms against an average of 0.05`. Scanning the airborne samples only, the identical signature was then produced deliberately (`1.27 vs 0.08`) by giving the width a 40ms sprint, which is the falsification the old spelling could not offer. **Frame drops were never this law's problem**: both terms are per-millisecond, so a coarse sampler moves `fastest` toward its average, and a law is allowed to degrade in that direction.
