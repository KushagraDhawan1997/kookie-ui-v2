...
Kushagra: *"What I want to create is very close to HIG by Apple. Do you see? Thats what Kookie is, its not just a UI library."* The site that existed was a scaffold — a component registry, three instruments, and a home page. What it could not do was state the system's actual content, which is a taxonomy and an argument, both of which already existed in `THESIS.md` and `DECISIONS.md` and neither of which was published. So the work is mostly PUBLICATION of an existing canon plus a voice change, from descriptive ("the system refuses two louds") to normative ("one focal action per surface, and here is why").

**The framing settled one thing immediately and it is worth stating.** A HIG can be code-free because Apple ships developer.apple.com separately, staffed, and every HIG page links out to it. We have no someone else — no third-party tutorials, no Stack Overflow corpus — so a guideline document here with no reference is a manifesto for a product nobody can install. The reference is therefore not dropped, it is SUBORDINATED: guidance leads, the generated API table is the last section on a component's page, and `refusals` sits above it. A missing prop and a refused one look identical in an API dump; only one of them is a design.

**MDX as a LOADER, not a framework.** `fumadocs-core` is a genuine headless docs core and was refused on the test this repo applies to Base UI: adopt HARD headless things. What it abstracts — a nav tree from a directory, a table of contents, prev/next, a content source — is a weekend of code we fully understand, so taking it would trade a shape and a release cadence for work we can do. What we DID take is the compiler, which is the hard part. Shiki likewise: TextMate grammars are decades of accumulated correctness, they are behaviour rather than appearance, and they are invisible until we paint them.

**And the paint is ours.** The syntax theme is generated from the ten tones through Shiki's CSS-variables mechanism, so every token colour resolves through `--{tone}-ink` — the SOLVED ink ladder (§15, 2026-08-10). That is not tidiness: a stock highlight theme would be a second colour system living inside the first, and the only text on the site held to no contrast target at all. Bold is dropped from the theme and italic kept, because `bold` is refused package-wide and a weight we do not ship would be synthesized by the browser — the font faking a token we deleted on purpose.

**Chapter metadata lives in TypeScript, not frontmatter, and that killed `content-collections`.** Chapter ORDER has to be authored — a directory listing cannot express it — so a registry exists either way, and frontmatter beside it would be a second home for the same fields. Keeping it in TS also means `tsc` checks it, which is how the component reference caught a documented `accentColor` prop that had never existed. Four dependencies deleted by noticing that the thing they provide is the thing we already have.

**Examples are REAL FILES, rendered live and shown verbatim — one file, two consumptions.** The 31 inline specimens moved out of the registry into `examples/<slug>.tsx`, which also made the registry pure data and let its coverage law stop being a regex over source and start importing the values. **Rejected: builder documents as the only source.** The builder's serializer produces JSX from a document under a round-trip law and was the tempting mechanism; it cannot express a handler, controlled state or an import, so a docs system built on it could never show a form that submits. Refusing to show people how to handle a submit is not a refusal this system has an argument for. Files are the base; the builder is the upgrade where the grammar fits.

**The API tables are generated from the package's own types**, with a drift law on `tokens.css`'s exact pattern (regenerate, compare, never write during `build`). Only the props the package DECLARES are shown — expanding the intersection through the checker yields 324 names for `FlexProps` and buries the six that carry the design under `onAnimationIterationCapture`. But the AGREEMENT law uses the checker's full answer, because a prop can be perfectly real and absent from the declared list: `gap` arrives from Box's shared table, `rows` from the platform. The first spelling checked the displayed list and failed on eight true sentences — a law that is wrong about the general case while looking right on the one its author had in mind.

**One plugin ships, and the reason it is exactly one is instructive.** GFM makes `| a | b |` a table; without it the colour chapter's ten-family table compiles to a row of pipes, which is how it shipped for an hour. Highlighting deliberately does NOT ride a plugin, because Turbopack requires plugin options to be JSON-serializable and our theme can only ever be an object — so `mdx-components.tsx` maps `pre` to the same CodeBlock the component pages use, and prose fences and example sources highlight through one function. The plugin is configured twice (Next and Vitest) and therefore owes an agreement law, which it has: the suite compiling MDX differently from the site would make every chapter law an assertion about a pipeline nobody serves.

**Search is ours, not Pagefind, and that is a downgrade taken deliberately.** Pagefind builds a real BM25 index over rendered HTML and is the better tool for a big site; it wants a post-build crawl of an output directory and somewhere to serve the index from. For fifty pages whose content we already hold as structured data, a substring match over titles, headings, blurbs AND REFUSALS is honest and is about eighty lines. Indexing the refusals is the part that earns its place: "why can't I set a margin" is a real search, and the answer is a refusal rather than a feature. Recorded as replaceable — the palette reads a list of results and does not care where they came from.

**Three prose primitives turned out to be unshipped, which nobody had noticed because nothing in this repo writes prose.** The system has no list, no inline `Link` (§11 has planned one since the defaults table was written) and no table (§11 plans a table ROW, an interactive surface; a static data table has never been specified). Each is handled in `prose.css` as the semantic element with its type from `<Text render>` and everything else in tokens, small enough to delete the day the component lands.

**The site moved into `Shell`, making the docs its second real consumer.** The builder was the first, and porting it found a hide mechanism dead since the day it shipped. The nav is `ShellNavGroup` + `ShellNavItem` with one docs-local block: a collapsible group standing in for the tree the package will ship (Kushagra: *"kookie will ship a tree too, not today"*), written as one part with two props so the migration is a deletion rather than a rewrite.

**And the builder is woven in at the layer that matters.** The composition chapter narrates the house style in its own words and then renders the reviewer's ACTUAL rule set from `RULES` — the same array the builder walks over a live document. The rules exist in two registers, prose and checks, and the ordinary outcome of that is drift in both directions. Now a rule added to the linter appears on the guideline page the same day, and the sentence a reader sees is the sentence the finding prints.

**The writing audited the system, which is this repo's own pattern arriving on schedule** (/preview forced the composition rules; the builder forced `componentAxes`). Generating the API tables surfaced 47 exported props with NO JSDoc — and the pattern was that they were the system's most central ones: `size`, `tone`, `emphasis`, `weight`, and the floating components' controlled-state trios, undocumented precisely because everyone assumes they are obvious, while `bordered`, `loading` and `leading` beside them all carried careful comments. Then the law that caught them turned out to walk registry ROOTS only, hiding 64 more on the compound parts — a law narrower than the rule it enforces, which is a law that cannot fail wearing a quieter disguise. Writing the chapters against the code turned up doc–code drift in `DECISIONS.md` §3, §4, §6, §7, §8, §10 and §11, all fixed in this batch.

**Two of my own decisions were wrong first.** The docs opened by organising themselves as `/components/<name>` — names first, which is THESIS §2's stated inversion, applied to the system's own documentation by the person quoting it. And the first chapter layout put a 46rem measure in the chrome, which is right for a chapter and wrong for the two other page shapes, leaving a third of the window empty on every page; the page states its own measure now.
