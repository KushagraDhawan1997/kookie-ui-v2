...
**Why it is not the ink, which was my own first proposal and the more interesting refutation.** The ink clears the non-text floor everywhere, so "just use the ink, it costs nothing" looked obviously right. Measuring killed it: the ink is solved for READING at Lc 60+, and meeting a higher bar is paid for in saturation. The solved glyph is **1.3× to 2.2× more chromatic than the ink in every family and both modes** — accent light `#0296ff` against the ink's `#2a6caa`, accent dark `#1999ff` against `#95c2f2`. The first pair is a vivid blue beside a muted navy, which is the whole difference between an accent and a value that merely has a hue. **The cheap answer was checked against the expensive one before the expensive one was built**, which is the only reason it is defensible to have built it.
