...
**The scroll adoption came out of comparing what shipped against a spec Kushagra had written and never committed.** Most of it matched without either side having seen the other — the not-a-Dialog/not-a-Menu split, the forced anatomy, the six exports, `modal` and `Arrow` refused, no drawn ✕, `size` pricing the box and not the content. Four things were missing and he waved three of them; this is the one he did not. It was a real gap: `popover.css` RECOMMENDED a ScrollArea to the caller, which is a component asking a call site to solve a problem only the component can see — `--available-height` is a fact the positioner reports to the pane, and a caller wrapping its own content cannot read it. An overlong panel clipped its own bottom with nothing to reach it by.

**`focusable` is where it deliberately departs from Menu, and the reason is the difference between the two panels.** Menu passes `focusable={false}` because ARIA voids `role="presentation"` on a focusable element and a roving-focus widget must not grow a tab stop inside itself — it can afford to, because the menu scrolls its own highlight into view. A popover traps nothing and holds content nobody else will scroll, so its viewport must be reachable (WCAG 2.1.1). Both directions are law-asserted in one mount, against each other, because "the two differ" is the claim.

**The corner divergence is worth recording because the spec's own argument does not survive measurement.** The spec says take the concentric corner whole, so that "a pane of this size opening beside a menu should not have a different corner from that menu". But the concentric derivation is *row corner + the pane's own padding*, and a popover's padding is the surface band, not a menu's 4px: measured, a popover under that arm comes out **42 / 54.25 / 71.75 / 89.25** against a menu's **28 / 33.25 / 36.75 / 40.25** and a card's **38.7 / 51.6 / 64.5 / 77.4**. It matches neither — it is rounder than both. The mechanism cannot deliver the goal it is prescribed for. What shipped (the card's band, via the `kui-floating-rows` split) stands; Kushagra confirmed the radius and padding calls.

**The dead hook, and the instrument lesson that came with it.** Nine flight declarations pinned a panel's body at `var(--kui-floating-p)` — the floating family's padding OVERRIDE, declared by menus and selects and by nothing else. On a popover or tooltip it resolved to nothing, the declaration was invalid at computed-value time, and every inset fell to `auto`. They read `--kui-sf-p` now: the RESOLVED padding of whatever pane it is, which on a menu resolves *through* the override by the size join and is therefore byte-identical there.

**A browser law for it was written, measured, found unfalsifiable and deleted.** `getComputedStyle` reports the USED value for an inset on a positioned element and can never answer `auto` — so with the hook unset the body falls to its static position, which IS one padding from the pane's top, and the used value reads the same 16px under both spellings. Reverting all nine changed nothing the law could see; sabotaging one to `99px` showed instantly, which is what proved the instrument fine and the property mute. **This repo had already recorded that trap** — the 2026-08-19 Tabs audit killed "drawn by both edges" for exactly it — and it caught me anyway. The guarantee lives in `surfaces.test.ts` now, where a declaration can be read, with three sabotages including the over-correction (deleting the override from the size join, which would silently give every menu a card's inset).

**An empty badge rendered a coloured dot.** `<Badge tone="success">{done && "New"}</Badge>` type-checks, and with the condition false it drew a 23 x 16 green lozenge with no text — which is the exact thing the component reference's own refusal names ("an empty badge, or a bare dot", WCAG 1.4.1). The TYPE cannot close it: `{cond && "…"}` is `string | false` and is a legitimate thing to write. So the component answers at runtime through `filled()`, the same helper the icon slots already ask the same question with. Rendering nothing rather than throwing; `0` stays, because a badge reading "0" is a real thing to say, and that half is law-asserted so the guard cannot become "render nothing, ever".

**The generator: 2381ms → 1365ms, `diff` clean.** Three solvers measured against the page and each rebuilt the entire neutral scale to get one hex, inside a forty-iteration bisection, per family per mode. One per-mode cache on `backdropCache`'s pattern, for `backdropCache`'s own stated reason (the call sites are spread across three solvers, so hoisting one leaves the next author to rediscover it). **It is build-time only** — the browser does zero colour maths (§7) — but `color.test.ts` calls the generator at module load and carries a note about having already timed out CI once at 5s, so the cost was landing on the one file with a history of exactly this.

**Two recorded rather than taken.** The glyph's floor contradiction — solved to a boundary against two beds, while a lit row is a third, so 10 of 20 lit cells and 16 of 20 pressed fall under the non-text floor the section justifies the role BY — stays open until it can be judged by eye, because the honest fix shifts every icon colour in the system. And the sRGB/P3 gamut split is DECLINED with its numbers (dE2000 4.22 between the two emitted values, ten of twenty cells): real, unjudged on a real P3 display, not the one-line repair it looks like, and every value clears its floor. Both are in §7 so a later audit finds a decision rather than a defect.

---

## 2026-08-23 The audit of the accent work, and what a degenerate fixture costs

**What.** An ultracode audit (13 agents, six lenses, one adversarial refuter each defaulting to *refuted*) over the eight-commit diff before it merged. 29 findings survived verification, deduping to 17. Four blocked and are fixed; the doc drift and the doctrine's own law are fixed with them. **The four component suites ran 74/74 green with every defect live**, which is the finding behind the findings.

**The blocker was mine, from the same day, and it is the lesson this repo keeps re-learning.** The new `.kui-row > [data-slot="leading"]` rule took a row's icon OUT of the inherited colour chain. Three vocabularies move a row's ink; I widened one — the disabled remap — wrote a comment explaining why that was the right home for it, and did not ask what the other two were. Under `contrast="high"` a lit row goes solid and its label follows to `--tone-contrast`; the icon stayed the glyph, painted ON the fill it sits on. Measured **APCA Lc 0.0 in 14 of 20 tone × appearance cells and under the non-text floor in all 20** (light neutral: glyph `#1f1f20` on solid `#030303`). It is a REGRESSION on `MenuItem`, whose `leading` prop predates the branch and which the shipped preview uses throughout. And it is verbatim the half-fix the same file warns about two blocks above the rule I broke: *"a fill that changes underneath ink that does not is the half-fix that ships."* The high-contrast arm now names every slot, which is the general statement it should always have been.

**Why 200 laws stayed green: `openMenu()` renders `<MenuItem>Alpha</MenuItem>`, with no icon.** Both high-contrast row laws are therefore degenerate for this rule — their fixture cannot tell a correct implementation from a broken one, because their fixture has nothing to break. That is the 2026-08-20 rule, and this is its clearest instance yet: the laws were not wrong, their INPUT was.

**Three more, all one shape — a dead control that is only dead on one element.** `Row disabled` was a complete no-op on the `render={<a/>}` path its own JSDoc recommends: full-contrast ink, `pointer` cursor, in the tab order, activatable, announced to nobody, while a `Button` on the same page got it right. `PopoverTrigger`/`PopoverClose` never inferred `nativeButton`, so a disabled trigger was a tab stop with no `aria-disabled` — **the 2026-08-03 Button defect on its fourth shipping**, with Chromium printing Base UI's own warning on that path the whole time and the fix sitting in `menu.tsx` under a comment naming it. And a disabled `ShellNavItem`'s label did not dim, because the stand-down I added that morning replaced a tone role (which the disabled remap rewrites) with a tone-less one (which it cannot reach): live words between a dead icon and a dead cursor. **Fourth appearance of that shape, and the first three are cited in the file I broke.**

**`rootsInButton` promoted on its fourth consumer**, the same fourth-consumer promotion `slot()` took hours earlier — AlertDialog, Dialog and Menu each carried a byte-identical private copy. Row is the one member with no Base UI primitive to delegate to, so it states the contract itself, and only where the target is not already a button: on a real one `disabled` IS the contract and `aria-disabled` beside it is a double announcement.

**The doctrine was guarded by nothing that could fail.** Both `DILUTED_ROLES` laws derived their expectation from the set they appeared to constrain — "is everything in the set neutral and everything outside it accent?" stays true whatever the set contains. The audit shrank it from nine entries to two, regenerated, and ran the whole suite: **1797 passed.** Accent's hover fill, press fill, three glass twins, faint ink and surface tint had all gone back to being faded blue. The membership is now written out independently and checked both ways, with `blue` as a per-role control rather than a per-group one.

**Doc drift, six items, four of which this diff created**: a published component-reference entry and a CSS comment still teaching "grey means the pointer is here, accent means this is where you are" — which the same diff's own rewritten law pins the OPPOSITE of; `apps/docs`' colour chapter untouched, still teaching that a tone moves all three ink rungs and omitting the glyph entirely; the Badge registry claiming tone "moves the ink and the fill together" (true for the nine meanings, false for accent — measured byte-identical to neutral's fill); `#0296ff` where the emitted value is `#0095fe`, in four places including two lines apart in DECISIONS; a `tooltip.tsx` comment saying it "carries no `data-size`" seventeen lines under the `data-size="1"` it stamps; and a `shell.css` comment block that had been UNTERMINATED, silently swallowing the comment below it.

**And one of my own fixes was caught by its own sabotage pass.** The law "a real button gains none of it" mounted a bare `<Row disabled>` — which never enters the `render` branch the guard lives in, so deleting the guard changed nothing the law could see. Re-fixtured on `render={<button/>}`. The degenerate-fixture rule, on the day it was quoted, in the commit that fixes an instance of it.

**Left open, deliberately, for a taste call rather than a patch:** the glyph is solved to the boundary against exactly two beds (page and seal), and a row's own lit wash is a third nobody solved against — 10 of 20 lit cells and 16 of 20 pressed fall under the non-text floor. The diff justifies the glyph BY that floor and then puts its headline consumer at 43.1 on it. Either the lit bed enters `worst()` or the claim narrows to the two beds it holds on; shipping the contradiction is what this repo's history says becomes the next audit's finding.
...
**Why it is not the solid.** Kushagra, closing the colour discussion: *"we anyway need to accept an accent color from user, my blue or any, and render tones suitable for icons, for buttons… and for text."* Two of those three already worked — the loud button's label is chosen black-or-white per colour (7 of 7 hostile brand colours pass in both modes) and the text ink is solved (7 of 7). The icon case did not exist: icons wore the raw fill, and the fill **misses 6 of 14 measured cells**. Our blue misses on the dark page (|Lc| 43.4 against 45); a brand yellow, green or pale pink misses on the light one (19, 27, 23). The pattern is the finding: **no single hex is legible on both white and black**, so a fill can never be an icon colour. That is a property of colour, not of this palette, and it is why the role had to exist rather than be picked.

**Why it is not the ink, which was my own first proposal and the more interesting refutation.** The ink clears the non-text floor everywhere, so "just use the ink, it costs nothing" looked obviously right. Measuring killed it: the ink is solved for READING at Lc 60+, and meeting a higher bar is paid for in saturation. The solved glyph is **1.3× to 2.2× more chromatic than the ink in every family and both modes** — accent light `#0095fe` against the ink's `#2a6caa`, accent dark `#1999ff` against `#95c2f2`. The first pair is a vivid blue beside a muted navy, which is the whole difference between an accent and a value that merely has a hue. **The cheap answer was checked against the expensive one before the expensive one was built**, which is the only reason it is defensible to have built it.

**What it is, as a sentence:** the accent moved only as far as the page forces. Light lands within a hair of the solid (`#0095fe` vs `#0094fc`) because the solid already clears there; dark lifts, because that is the only direction legibility exists. Chroma never moves — which is what makes the glyph legal under the same day's `undilutedTones` rule, and why an undiluted tone keeps its own glyph rather than pointing at neutral's. Fading is banned; placing is required.

**Mechanism.** `solveGlyph` in `color.ts`, `solveControlEdge`'s shape with the family's hue in place of neutral's. Chroma peaks at the hue's cusp and falls away in both directions while contrast rises monotonically away from the bed, so the answer is the cusp when the cusp clears and otherwise the nearest point to it that does — bisection finds it exactly, no scan. Contrast read on the sRGB rendering even when emitting P3, following the `contrast` decision's own sentence. No `contrast="high"` variant: Lc 45 *is* the WCAG 1.4.11 bar.

**The mark family's ON state deliberately did NOT follow the tick**, and the divergence is principled. A checked box is a filled AREA and answers `nonTextLarge` (30), which the solid clears with room; a tick is fine detail at 45. §7 already states exactly that split for a field's border against a checkbox's ring. What the old comment's claim of "one selected colour across the system" survives as is one selected FAMILY, which is what it was reaching for — the comment justified itself by the non-text floor and then read a value that missed it, for two weeks.

**Two mechanisms caught the change rather than me.** `preview.ts`'s `roleValue` is exhaustive over `ROLES` with a `never` guard, so the new role failed compilation until the swap could express it — the 2026-08-06 note on that function says it replaced a hand-kept ten-name list that had already drifted, and it just paid for itself. And seven browser laws failed on the consumer moves, every one asserting "the accent solid"; all seven now assert the glyph and all seven fail again when the CSS is reverted.

**Laws.** Four node laws — every family's glyph clears the floor on both beds in both modes; it is the FAMILY and not merely a value that clears (chroma held above the ink's, so a solve returning black fails); the emitted role carries it with accent keeping its own; and **the solid still MISSES**, which is the law that makes the others mean something. A floor law passes on a system that never had a problem, so the thing that was broken is measured and asserted to be still broken — if the solid ever starts clearing, the role is redundant and somebody finds out here rather than never. Falsified four ways (glyph returns the solid, returns the ink, maximises contrast instead of chroma, floor dropped to 5) plus a CSS revert for the consumers.

---

## 2026-08-23 Accent is never diluted — it is a role, not a family

**What.** An undiluted tone points its WASHED roles at neutral through the tone indirection: the soft trio, its opaque glass twins, `a3`, and the two faded inks. `undilutedTones` in `color-config.ts` holds the policy, `DILUTED_ROLES` in `generate.ts` holds the role vocabulary, and today the set is exactly `accent`. So a medium accent button, a quiet one under the pointer, a current sidebar row, an accent badge and a tone-forward accent surface are all filled in grey, and the accent moves to the label. Loud keeps the pigment. **Zero component edits — every consumer was already asking the indirection rather than naming the family** — and it came out 40 bytes cheaper (31,778 → 31,738), because the accent block now shares its text with the neutral block.

**Why, and the measurement that made it a rule rather than a taste.** Kushagra, after two days on what colour means in this system: *"an accent never paints a 'faded' background. Accent is pure. You pick accent, and its the vibrant color you pick. Your loudest buttons get it. Its the medium emphasis button, and quiet on hover, that lose that."* Measured, the soft rung was never a quieter accent to begin with — in light it is the pigment at **11% alpha** over 89% page, and in dark it is a *different colour*: `srgb 0 0.157 0.294` at 90% against a solid of `#0094fc`. Two hexes that share a hue. So this is a correction, not a preference: the system was calling something accent that had already stopped being accent.

**The distinction that survives the rule is FADING versus PLACING, and getting it wrong would have deleted accent text.** A wash spends chroma to get quiet. The ink rungs spend nothing — they hold full chroma and move LIGHTNESS until the hue reads on its bed, which is why the ink INVERTS between modes (`#2a6caa` light, `#95c2f2` dark) while the solid is one hex in both. A dilution tracks its source; this does not track it at all. That distinction is load-bearing because the pure accent **cannot be text**: measured Lc 56.1 on the light page against a body floor of 60, and |Lc| 43.4 on the dark one. Every blue word in this system is a placed value or an illegal one.

**Accent-only, and the asymmetry is the decision rather than an exemption.** The other nine keep their washes: they are MEANINGS, and a pale red still reads danger, so diluting one costs it nothing it was carrying. Accent is the one family a consumer chooses, and 11% of somebody's brand is not their brand. The repo had already half-decided this without noticing — `button.browser.test.tsx` and `kbd.browser.test.tsx` both test through `blue` rather than `accent`, with the comment *"accent is a CONFIGURED identity that may equal any family"*. What the change really does is finish that thought: accent stops being a member of the tone set and becomes a role wearing a family's clothes.

**Stated cost.** `accent ≡ blue` stops holding at the diluted roles — §7 claimed it by construction and it was worth 539 bytes. It was true while accent was a colour and it stops being true the day accent is a role, which is the right direction for it to break.

**Stated consequence, left unfixed on purpose.** A current row at rest is now byte-identical to any row under the pointer, because medium's rest and quiet's hover are one neutral wash. What says "you are here" is the LABEL plus `aria-current`. Two shell laws caught this — they were written the other way round, in their own words *"grey means the pointer is here and accent means this is where you are"* — and rather than deleting them they now **pin the collision**, so buying the distinction back with a fourth rung must re-open this decision instead of quietly restoring it. Rejected on the spot: stepping the row's current fill one rung deeper (runs out of ramp at press, and it is a taste call that belongs in the playground, not in the commit that changes the doctrine); and dropping the current row's fill entirely, which composes perfectly and makes "you are here" nothing but blue text in a list.

**An instrument finding worth more than the change.** The first sabotage pass on the mounted laws reported **all three mutations surviving** — the laws looked worthless. They were not: browser laws read `tokens.css` from DISK, so mutating `generate.ts` changes nothing they can see until `pnpm run tokens` runs. Regenerating between mutation and run turns each mutation into six failures. CI is safe because the build regenerates first, but a hand-run sabotage on any generated-token law is a silent no-op without it — the 2026-08-08 *"a law that did not run is a way of not failing"* finding wearing a different hat, and now recorded in the file's own header.

**Laws.** Four node laws (the emitted accent block role by role; a diluted twin as the negative control; the meanings keeping their washes; and the cheap one that catches a typo'd role name making the whole mechanism a no-op) and five mounted ones per appearance. **Every mounted law carries `blue` as a negative control**, because accent and blue are the same recipe under two names — so a generator that had sent every tone's wash to neutral, or a stylesheet that had stopped painting fills at all, satisfies "accent's medium fill is neutral" perfectly. The pair differing is the claim. Falsified four ways at the node level and three at the browser level.
...
**Why now, and the evidence was in this repo's own apps.** Two screens were hand-rolling it. `docs-search.tsx` rendered results as full-width quiet Buttons and said so in a comment — "the package does not currently export a row that lives outside Menu or Shell; a quiet full-width Button is the honest stand-in until it does". `command-palette.tsx` painted its highlighted row `var(--neutral-a3)` inline, which is the row family's lit fill redrawn by hand, and it had already drifted: that fill grew material remaps and a high-contrast solid arm on 2026-08-19 while the copy stayed one flat step. Both are ported in this change. A component whose reason for existing goes unspent is the entropy this repo keeps paying for.

**The design question was what lights a row, and the answer §21 guessed at was the wrong SHAPE.** The open question proposed "a Kookie-owned lit attribute the family stamps, with Base UI's two mapped onto it at the Menu boundary" — a translation layer at a component boundary. But the distinction is not per component. The same `Row` is roving inside a command palette, where the arrow keys move a highlight while focus stays in a field, and not roving in a settings list, where the pointer is the only cursor. No component boundary can answer a per-instance question.

So the claim sits on the instance: `Row` stamps `data-hover-lit` when nobody else is driving it, and stops the moment a caller states `highlighted`. Stating it AT ALL is the signal — not its value — because a controlled `false` still means "I am the cursor here". The shared layer carries one rule and it wins on SPECIFICITY, (0,4,0) against the stand-down's (0,3,0), which is a real improvement on the mechanism above it: that one wins on source order and its own comment calls that fragile to leave undocumented.

**The direction of the marker is the part worth defending, and a LABEL is what settles it.** The other spelling — gate the stand-down on "is this row roving" — delivers the same result for `Row` and is wrong: a menu label is a `.kui-row` that is not a control at all, so gating hands hover to every row that never asked, headings included. An opt-in marker leaves anything that says nothing inert. Stated generally: answering the pointer is what every control here does, a row that must not is the special case, and the special case marks itself.

**`shell.css` predicted this in writing and its comment is now spent.** It carried `.kui-shell-nav-item:hover` from the day Shell shipped with the expiry attached — "the second member self-keys, and a third non-roving row is what promotes it" — and §21's open question had predicted the shape of that private rule a week before it was written. Two shell laws encoded the old state and came due: the sanctioned-`@media` set went from three forms back to two, and the "the shell paints no bed" law's second exemption died exactly as its own comment said it would. Neither was weakened; both now assert the new guarantee, and one of them asserts the private rule cannot come back.

**Refused.** A `List` component to put rows in — a column with a gap is a `Stack`, and which role a list needs (`list`, `listbox`, `menu`, none) depends on what the rows mean, so a wrapper would have to guess; this is `Progress.Track` and `FieldControl`'s own refusal. A keyboard model — the list owns its highlight, because only the list knows what its items are and what Enter does to them; `Row` paints what it is told. A `selected` prop — picking one of several is a radio group (§26), and `current` is a different thing that IS shipped: it announces `aria-current`, it means location, and it is not a form value. `emphasis`, inherited from the family. And a button-height row: `Row` is the panel row, and the standing row that sits level with a button is `ShellNavItem` — whether a standalone one should be able to stand is recorded open rather than guessed, since withholding is the additive direction.

**Four of twenty laws were wrong before they were right, two in shapes this repo has already named.** The comparison against a menu row read the FIRST popup in the document rather than the last — mounts accumulate, so four laws were comparing a row against one from a previous mount — and read it on the mount frame, MID-FLIGHT, where size 3 measured 89px against the row's real 34. A third asserted that a menu row does not light under the pointer, which was never true and was never the claim: Base UI stamps `data-highlighted` on hover as well, and the stand-down exists so the row goes dark when the keyboard moves the highlight off a pointer-rested row. That law reads the marker's presence across every member now. The fourth asserted a strict inequality in a cell that is a designed equality (coarse size 1: row and button both 36px), and it is asserted as an equality rather than skipped — Switch's coarse size-4 rule.

---

## 2026-08-23 Badge, and a floor law that had never measured a floor

**What.** The inert-atom family's third member ships, and its arrival moved the family's shared identity into the type layer: `.kui-atom` (fill, corner, the inherited-size arm) and `.kui-atom-box` (the one-line box). `badge.css` is two declarations. +17 bytes gzipped, the cheapest entry the repetition curve has recorded.

**Why a third component in a family of near-twins is not one component too many.** Code quotes a literal in the mono face and belongs to its sentence; Kbd names a key in a box with an edge and relief, because a key is a physical object; Badge is that same box with neither, and what it carries instead is the TONE. The flatness is the argument, not a saving: a cap stands proud of the surface and a badge is printed onto it. It is law-asserted against a mounted Kbd in the same world, because "nothing casts anywhere" passes for it otherwise.

**Tone is the category, not the volume**, which is §29's sentence arriving one family over and is what kills the `variant` every peer ships here. Emphasis takes the type resolution — foreground roles — so the box holds while the letters step. A `loud` badge would be a second axis reading the same word, which is §9's reason for deleting `variant` in the first place. It is also the first component whose whole point is to spend the semantic families: Notice consumes them one at a time, a badge puts ten in a column.

**The promotion is what made it cheap, and the promotion is also the only thing here that could rot.** A member that quietly re-declared the fill would look right for exactly as long as its copy happened to agree — and no mounted law can catch that, because a copy that agrees computes the same value. So the node laws read the SOURCE: the shared identity is declared once in the type layer, no atom's own sheet declares any of it a second time, and no two atoms read one scale token. That last one matters because `--badge-scale` equals `--kbd-scale` today: the atoms deliberately do not share a discount, since mono's is a metric correction, the cap's is about ⌘ drawing full-size in the sans, and the badge's is about rank. `segmentInset` and `switchInset` set the precedent (§26).

**The finding worth keeping is in a law that was already shipped.** Badge's floor law — "a single character stands at its floor, which is the count case" — was copied from Kbd's, and its sabotage pass survived: deleting `min-inline-size` from the promoted box changed nothing the assertion could see. The fixture was the defect. `1` renders 1.634em wide and `K` 1.654em, both CONTENT-driven and a hair over the 1.6em floor they were supposed to demonstrate, so the floor was never what set the width being measured. Only `i` and `.` are floored (1.599em); `W` overruns at 1.987em. Both laws now assert the ratio exactly on a narrow glyph, with a wide one beside them proving the floor is a floor and not a fixed width. This is the degenerate-fixture rule (2026-08-20) catching a law that predates it, and the mechanism by which it was caught is worth naming: **copying a law copies its blind spot, so a copied law owes its own sabotage.**
...
**Why it came up.** The first was raised by the 2026-08-23 floating-motion audit and measured three ways by its verifier. The second is Kushagra's, the same day: *"material behind menu jumps after it settles — the refraction takes place after animation finishes, with a jump"*.

**The map, and why "stretched" was a comment rather than a fact.** `useLens` generates at a capped resolution — the bend is a low-frequency field, so this is what keeps a full-page pane cheap — and three separate comments in `refraction.tsx` said the result is "stretched to the box". It never was. The `feImage` carried the map's own generated size as its primitive subregion, in px, and a subregion PLACES an image at that size; it does not scale it to the filtered element. A 600x420 glass card minted a 320x224 map and drew it in the corner. What made it visible rather than merely absent is what lies outside a subregion: `in2` is transparent black, `feDisplacementMap` reads 0 on both channels, and 0 is a uniform half-scale shift — so the rest of the pane was flatly displaced, with a measured ~3.1px seam down the middle of the box and no bend at all on the real right and bottom edges. §10's whole legibility argument for the near-clear ladder was missing from most of any large pane.

**The repair is a deletion.** The filter region is stated as the border box (`0 0 100% 100%` — `filterUnits` already defaults to `objectBoundingBox`) and the primitive inherits it, so no size is written in px anywhere. That is what makes it right at every size rather than at one. `primitiveUnits` is deliberately left alone: it stays `userSpaceOnUse` so the displacement `scale` keeps meaning pixels, because the strength is derived from the physics and has to stay in the physics' unit.

**Stretching alone would have shipped a second defect, and the verifier caught it before it was written.** The bezel and the glass depth are LENGTHS drawn in the map's own pixels, so a half-size map stretched up renders a lip twice as wide, and wider again on a bigger pane — the lens would stop being the same lens at different sizes, which is exactly what the judged values exist to prevent. So the physics crosses the cap with the map: generate with `bezel * scale` and `thickness * scale`, and convert the bend back with `max / scale`. Measured, an uncapped pane's strength is unchanged to fifteen digits (13.868163690327355 before and after) and a capped one now matches it to 0.04%, where leaving either half out puts it at 7.39 or 26.00.

**The pop, and why it was not a trade.** The 2026-08-22 audit found 27 filters minted during one glass menu open — a per-pixel Snell solve, a PNG encode and an eleven-node filter graft, every frame — and repaired it by building nothing until the flight ended. That repair is what a person sees as the jump: measured across a real open, 70 of 90 frames carried no `--kui-lens` and the value changed on the landing frame. Both are now avoided by the same fact rather than by choosing between them. A flying panel's own box is the wrong box twice over — a frame stale, and its corner mid-transition between the seed's and its own — so the runner publishes the box the flight is heading TO. `--kui-fly-r` joins `--kui-fly-w/-h`, read off the un-posed element in the one window where the resting corner exists at all. The lens builds once, for the geometry the panel will land wearing, and wears it the whole way; the landing seam becomes a cache hit and the filter on the element does not change. After: 0 frames without a lens, no change at the seam, and the same two filters in the document.

**The seam is still watched, and that is deliberate.** The equality is not guaranteed by construction — a panel whose content changes size mid-flight lands somewhere the flight never published, and it must not be left wearing a map for a box it no longer has.

**No law could catch any of it, and both new ones were falsified.** The lens's existing laws read that a filter is minted and that its reference resolves — true of a map drawn in the corner. The card law now reads the emitted filter (the register the defect was in: what the generator wrote) with a calibration that the pane exceeds the cap, plus a strength comparison against an uncapped pane of the same shape, which is the half a structural read cannot make. The menu law reads a panel while it is FLYING and decodes the map's own pixel height out of the PNG, because once the filter is size-independent that is the only surviving record of the box it was built for; a law that read the settled panel would pass on the defect, since the settled panel always had its lens.
...
**Why it came up.** The author reported trouble with these components' motion while all four suites were green — 199 of 200 — which is the finding behind the findings.

**The one mistake, three times: the flight does not own the element's style.** Base UI writes five properties imperatively on an item-aligned Select — `position`, `maxHeight`, `overflowX`, `overflowY` on the popup and `height` on the positioner. The runner borrowed and restored exactly one of them, with a paragraph beside it explaining why an inline declaration beats every rule, and then twelve lines later DELETED another outright on the premise that the positioner "shrink-wraps the popup" — true of a menu's, false of a select's. The generalisation is the fix: snapshot and restore, never enumerate what you think is there. The third property is repaired in the same currency rather than by fighting the cascade — the runner writes `overflow: clip` inline for the flight and hands back what it found, because an `!important` in the family's sheet would win and leave the library's intent unstated, which is the reason it was refused for the borrowed height in §23.

**The width floor's two causes, and why fixing either alone leaves the step.** An open trigger holds its press and the press is a SPRING, so the scale at the instant the entry measures depends on how the panel was opened: a real pointer press has not started travelling (measured `scale: 1`, rect 400 on a 400px button) while a `defaultOpen` panel is already holding it (`scale: 0.975`, rect 351 on a 360px trigger). So the division stays — it publishes the layout box in both — and the 2026-08-17 reversal that removed it was argued from a premise that is simply false: Base UI does not read `getScale` and normalise, the name appears in its positioning file zero times. Separately, floating-ui keeps re-measuring the anchor while the press travels, so a floor handed back at release is handed to a number still in motion; `--kui-anchor-w` is now the one flight var the release keeps.

**Rejected, and each after building it.** Shortening the overlay exit's restated geometry to the exit's own clock, to close the ~500ms window where a dismissed alert is invisible and still mounted: built, measured as a NO-OP (no value changes on those channels at the exit, and an unretargeted transition keeps its duration) and reverted rather than left in as a plausible-looking fix — the restatement's recorded reasoning is still exactly right. And any z-index for the un-rooted portal's missing stacking frame: measured real (a fixed `z-index: 50` cover paints over an open menu with no root `<Theme>` and under it with one), but every repair that works from inside the portal is a ladder big enough to out-rank the app's, which §20 rejected on the record and which would lose to the next app that picks a bigger number. The frame IS the mechanism; what was missing is that nothing said so when it was absent, so that one gets a dev warning naming the remedy and nothing else.

**Why 200 laws were green, and it is one line of test data.** Both Select fixtures used eight options. Eight rows fit the available height, which is the one shape where the whole item-aligned mechanism is a no-op: Base UI writes no constraining positioner height, the panel never scrolls itself, and the 57px trigger is discarded by `max(--floating-min-w, anchor)`. Three separate laws were asking their question of an input that answers the same whether the mechanism works or not. Widened to thirty, one law immediately failed on CORRECT code — it asserted `scrollTop <= 1` on every frame, which an item-aligned select is supposed to violate once its list scrolls, so the law had codified the defect as a requirement.

**The laws written for these fixes were wrong before they were right, in this repo's own recorded shapes.** A "does not step at release" law was put in the Select file where a field-shaped trigger never presses, so the disagreement it exists to catch cannot occur — it moved to Menu, and Select keeps only the mechanism half. Two Menu laws selected "the first" or "the last" popup while a previous law's panel was still mid-dissolve; a Select law did the same, which is worse there because a select keeps its panel MOUNTED for the life of the component, so every earlier law has left one in the document. One dismissed a panel that was still POSED, whose `transition: none` is the seed's and has nothing to do with the rule under test. One waited for "not unfurling" on a panel whose entry had not STARTED. Two carried a 0.5px tolerance against a 10px defect. Two lens laws could not fail at all and their own sabotage passes caught both. Passing alone and failing inside the file was the signature almost every time.

---

## 2026-08-21 A shell pane is a surface, and it pads — the rule was forced once and generalised past its case

**What.** Every shell pane now pads on all four sides, off Card's own ladder, priced by the index the pane was given. `shellNavInset` is deleted. A header states its height. The whole change is a net deletion of 14 gzipped bytes of CSS.

**Why it came up.** A docs session measured the header for an unrelated reason and found it 28px tall with its contents against the top edge, its only inset the 12px the docs app was drawing itself. Kushagra, looking at the same screen: things are cramped everywhere, and every pane needs air — a safe area.

**The rule that was wrong, and the exact shape of the mistake.** `.kui-shell-pane { padding: 0 }` shipped with "a pane's content owns its own inset" written beside it, and that sentence is FORCED for the side panes: a highlighted nav row must span its column, and a `ShellScroll` must reach the pane's edges. Air the ROW cannot escape breaks both. But neither of those needs the PANE to have none — and both escapes already existed, built for other components and never reached for here. A `ScrollArea` that is a direct child of any surface already bleeds to that surface's edge and re-pads its viewport; that is how a Card holds a list. Anything else that wants the whole box says `m="bleed"`; that is how a Card holds a picture. **This is the repeated shape in this repo — a rule argued where it is mandatory, then applied where it merely costs** — and it is the same sentence the docs session's own report reached independently.

**The consequence nobody had to build.** With the pane actually padding, the 2026-08-20 bleed defect stops being a defect: a `ShellScroll` rendering at x = −16 and 319px wide inside a 288px pane was correct behaviour against a pane lying about its padding, and the fix that forced the hook to zero was treating the symptom. That stand-down is deleted rather than adjusted. Nothing was added to make scrolling work; the shared rule already did it.

**`shellNavInset` is deleted, not renamed.** It was 4px, drawn by each nav row and rail square as its own margin, and it existed only because the pane had no padding to give. Two numbers doing one job — and on Card's rung they would have stacked to 20px. The row's margin goes; the pane's padding is the air. The press-area expander that gives a row its full column back reads `--kui-sf-p` now — the surface padding hook, which inherits deliberately, because that is how `m="bleed"` finds its nearest surface — so the inset and the reach that undoes it are one number by construction rather than two kept equal by hand. The rail's width formula loses a term with it: the extent is the square, and the padding is the air.

**A header's height falls out of the same decision.** One control row at the pane's index, inside the pane's padding — so at any index a header is exactly as tall as the rail is wide, and the frame's corner is square. Three spellings are load-bearing, each falsified: **content-box**, because under `border-box` a `min-block-size` of 32 is satisfied by 23px of content plus its padding and seam, so the stated row never binds and the header goes back to measuring its tallest control — the defect wearing a token; **grid rather than flex** for the centring, because a header's usual child is one full-width row and a flex container makes it shrink-wrap, which would have collapsed every existing consumer's header; and the index having to REACH the header at all — `ShellHeader`, `ShellContent` and `ShellBottom` stamped no `data-size`, so nothing priced per index could key on them. All six panes resolve and stamp it now, through one helper rather than four copies of the resolution.

**Predicted and deliberately not solved.** Card's padding ladder nearly triples across the index (12/16/24/32) while the control ladder does not quite double (28/32/40/48), so a rail or a header at size 3 comes out 88px and at size 4 comes out 112px. That is the fraction wall's shape — borrow a ladder built for a big box and it swamps a box whose whole size is one control — and it is the ninth time this repo has met it. Kushagra's call was to start on Card's rung and change if it needs changing, so the arithmetic is recorded with the four cells named rather than pre-solved. Sizes 1 and 2, which is every consumer in this repo, look right and were checked in a real browser.

**Four existing laws failed, and each was encoding the old behaviour rather than catching a break.** The bleed law asserted the hook stood DOWN to zero — right while a pane claimed no padding, a defect the moment it had some — and is inverted: the scroller reaches the pane's wall AND its viewport re-pads inside, both halves, because a bleed with no re-pad is content against the wall. The other three were arithmetic that moved. Rewriting them surfaced a small instrument lesson: all three first rebuilt the padding box from `--border-width`, and were wrong by exactly one pixel on the leading edge, because a flush pane draws ONE border and which side carries it is a fact about its neighbours. They read `clientLeft`/`clientWidth` now — the browser's own answer to the question, rather than the law restating a rule it does not own.

**The corner went with it, and it should have gone in the first pass.** It was pinned to `--radius-surface-3` with a reason written beside it: a shell composed inside another surface must not inherit that ancestor's corner, and `--kui-sf-radius` inherits. That reason died the moment every pane stamped `data-size`, because the size join then declares the hook ON the pane and an ancestor's value is shadowed rather than out-shouted. I left it out of the first pass and told Kushagra it was one line whenever he wanted it — which was over-correction, not judgement: earlier the same session I had changed something nobody asked for, and the response was to start asking permission for things that follow directly from a decision already made. The direct consequence of "size prices the pane" is that size prices the pane's shape too. **The spelling is the interesting half**: the drawer's two arms have to restore the corner as a PROPERTY (the flush rule zeroed the property, and `initial` on a property is zero), and re-pointing the hook instead is unsafe — a `.kui-shell-pane[data-flush]` arm and the size join's `.kui-surface[data-size]` arm are both (0,2,0), so the winner would be whichever file loaded first, which is the trap this repo has walked into repeatedly. So they restate the shared layer's expression, multiplier included, and the existing drawer law reads their AGREEMENT with a pane the app pulled off the frame rather than a number — which is why it caught both sabotages of this change without being touched.
...
**How it started, and the wrong answer I gave first.** Asked what to build next, I recommended Callout on a vocabulary argument: four semantic tone families ship, each costs ~1.15KB, and nothing consumes them. That argument was true and it was an argument for *spending the tones*, not for that component. Kushagra: *"before we even discuss callout, I want to know what would apple do… read our thesis."*

**Apple's answer is that there is no such component.** Read the full HIG component set from Apple's own data — eight categories, ~45 components. No callout, no inline alert, no banner, no toast. Every message component Apple ships interrupts (Alerts, Action sheets, Notifications), and the Status category holds four instruments only. The one box they ship is **Boxes** — *"a visually distinct group of logically related information"* — which is grouping, and is what we shipped as Surface the day before. Their instruction for the non-interrupting case is explicit on the Alerts page: *"If you need to provide only information, prefer finding an alternative way to communicate it within the relevant context… you could show cached or placeholder data and a nonintrusive label that describes the problem."* A label, in place, with colour carrying rank (*"Use system-provided label colors to communicate relative importance"* — four label colours, our ink ladder).

**But the pattern is real, and the specimen settled it.** Kushagra, pointing at Claude Code's own composer: *"it says 'Approaching weekly usage limit', that's a callout or alert, and there's a system for it, isn't it? Contextual, persistent information that can be dismissed."* Correct. My split had found three functions under one name and then only built the case for the one Apple had a clean answer for.

**The carve: two questions generate the family.** What is the message about, and what made it appear. Answer both and placement, lifetime, whether it takes layout space, dismissibility and the ARIA role are all forced — there are no free choices left. Written into §29 as a table. The joints are corroborated rather than asserted: Atlassian splits by scope (Banner = product-level, SectionMessage = *"above the affected area"*), Polaris splits by outcome-of-an-act versus condition-on-a-page, Apple refuses the box entirely. Three independent systems cutting at the same place is §4's recurrence test.

**Notice, not Banner, and the reason is §3.** Atlassian needs two components because their distinction *is* where it sits. Ours is one component the caller places — in flow above a region, or handed to the Shell. A component in this system does not own its outer position, and that non-negotiable turns out to delete a component rather than merely constrain one.

**Toast: refused, and both of my counter-arguments collapsed under one sentence.** Kushagra: *"If an action deserves attention, it must be done so before the action."* Attention is binary — either it is worth interrupting for, in which case you interrupt while the person can still change their mind, or it is not, in which case you do not narrate it afterwards. A toast sits in a category that does not exist.

I argued two cases for it and tested them against that rather than defending them. **Undo**: I claimed a toast is where undo lives on a platform with no system-wide undo. But an app with a real undo stack has it in the menu and on ⌘Z for as long as the history lives, so the toast adds only discoverability; and an app *without* one is being handed a four-second shape that resembles reversibility. Apple does not need toasts because Apple owns the undo gesture at the OS level and can promise it in every app — that is the actual asymmetry, not proactivity. **Copy to clipboard**: I claimed the operation is invisible so something must say it. But the best implementations turn the copy button into a checkmark in place — the control that did the thing reports the thing, where the eyes already are. Both cases dissolve, and what survives is thinner than I argued.

The refusal is in character (this system has already refused `variant`, elevation, `bold`, and both halves of the look axis) and it owes three things, recorded as obligations: Notice must cover conditions nobody triggered; a control must be able to report its own outcome briefly (a Button done state — **recorded open**); and the stance is published, that we will not lend a disappearing window that resembles undo.

**One structural concession kept:** if we shipped `toast()` as a comfortable one-liner we would teach the misuse, because THESIS §7 says the default *is* the teaching. That argument survives the refusal and is part of why refusing is cheaper than narrowing.

**The background job is not a message, and Kushagra named the trap before I could fall into it** — *"not by 'to a hammer everything is a nail'. Bc we are discussing this family, we might be inclined to solve it with just these affordances."* Real case, from Womp: people miss background downloads. A message is an event; a job is an object with a lifecycle whose artefact outlives it, so representing one with the other is a category error rather than a tuning problem. "People miss it" decomposes into five misses (did it start, is it stuck, did it finish, where is the result, what did I export before) and only the third is a notification at all. Every OS solved it with a **location** rather than an announcement — a fixed frame affordance carrying aggregate state that opens a list — which inverts the failing property: a toast leaves whether you looked or not, a badge waits. The real repair is usually the data model, because a durable result makes the moment free to miss. The one miss nothing in-flow can close (they switched tabs) is the browser's Notifications API, outside this library. **This is the first forcing case Badge has had.**

**The icon slot is a decision, not an omission** (Kushagra): §8 ships no icon set, but the slot exists anyway and renders nothing when unfilled, because a repeated symbol carries shared meaning across notices in a way a sentence cannot. apps/docs already uses Hugeicons at stroke 1.5.

**Field's four open questions closed the same session (§28).** Stacked layout, with horizontal refused on Slider's `orientation` precedent — a horizontal field is a designed grid (the label aligns to the control's first line, the error must sit under the *control*), not a direction flip. `size` on Field, reaching the control by context with an explicit prop winning, which is the material mechanism reused; named as the riskiest half, because silent inheritance is the `--kui-h` trap on its fifth appearance, so it gets falsified before it is trusted and the fallback is API-compatible. `Form` deferred, not refused — Base UI's distributes server errors by field name and moves focus to the first invalid one, which is non-visual behaviour, but it is additive and waits for something with server errors.

**And one call where I was overruled with the evidence on my side, recorded so it is not re-derived as new.** I proposed the description *and* the error both above the input, on GOV.UK's ordering (label, hint, error, input) and their perception argument: under a screen magnifier or at high zoom, the focused input is on screen and anything below it may not be. That is a floor argument from the most user-tested public form system there is. Kushagra took **description above, error below** on the consistency of this family's own before/after rule — instruction before the act, diagnosis after. The cost is written into §28 rather than buried: at high zoom an error below a focused input can be off-screen. The counter-pressure runs the other way and favours what shipped, since an error appearing above pushes the input down while the person is still typing, and Apple's own guidance makes live password validation a common case. If a measurement bites, the order moves and the trade was knowing.

**Rejected on the way:** Callout as a component (three functions under one appearance — §3's segmented-control-and-tabs error, and the name comes from documentation tooling, which is why every design system has one: a docs site is every design system's first consumer); Banner as a name (position, not function); Alert as a name (taken twice); a `Notice`/`Toast` pair distinguished by a prop (one takes layout space and one must not, which is structural); and solving the background job inside this family.

**The refusal is not yet complete, and our own app is what showed it.** Sweeping apps/docs for drift turned up the builder's local toast, whose four messages are all one shape — *"Nothing here accepts that"*, *"That file is not a builder document"* — the system refusing an act, with a reason, and no field to hang it on. The before/after rule does not reach that case, because the refusal IS the answer and cannot precede the attempt. Recorded in §29 with the likely answers (state it during the gesture where it is knowable then; a Notice or an AlertDialog where it is only knowable on release; and for the plain case, Apple's snap-back — the object returning is the message, which is "the row disappearing is the confirmation" one gesture over). The builder's toast stays until that closes: removing it is a behaviour change to a shipped app and does not belong in a taxonomy commit.
