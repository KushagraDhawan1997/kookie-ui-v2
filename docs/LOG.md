...
11. **A menu is born from its trigger, and the growth origin sits OUTSIDE its own box** — at the button, so it streams out from under the finger rather than zooming from its own corner. iOS 26 goes all the way: the button *morphs into* the menu, one body, with the glass **materializing** en route (backdrop blur and fill condensing — becoming solid, not becoming visible). Built and judged: the animation quality is the target, but the full body-swap is declined for the web — the unfurl-from-trigger with condensing material is the keeper.
